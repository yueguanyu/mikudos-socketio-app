...
class DUPLEX_HANDLER extends handler_base_1.HandlerBase {
    constructor(app, serviceClasses, { eventPath = 'stream-call', authenticated = true } = {}) {
        ...
        serviceClasses.forEach(dsc => {
            debug(`register duplex service: ${dsc.serviceKey}`);
            this.namespaces[dsc.serviceKey] = new dsc(this, app);
        });
    }
    register(socket) {
        debug(`register duplex services`);
        socket.on(this.eventPath, async (data, callback) => {
            const [namespace, method] = String(data.method).split('.');
            this._saveSocketHandledMethodsToHandledMethods(socket.id, namespace, method);
    ...
    _saveSocketHandledMethodsToHandledMethods(socketId, namespace, method) {
        var _a;
        if (!((_a = this.handledMethods) === null || _a === void 0 ? void 0 : _a[socketId]))
            this.handledMethods[socketId] = [];
        this.handledMethods[socketId].push({ namespace, method });
    }
    cancelAllOnSocket(socket) {
        ...
        lodash_1.unset(this.socketStreams, socketId);
        return { result: { success: true } };
    }
}
